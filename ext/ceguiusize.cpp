...
{
	return wrap(CEGUI::PropertyHelper<CEGUI::USize>::toString(*_self));
}
...
VALUE _equal(VALUE self,VALUE other)
{
	if(self == other)
		return Qtrue;
	if(!is_wrapable<CEGUI::USize>(other))
		return Qfalse;
	return wrap(*_self == wrap<CEGUI::USize>(other));
}
...
{
	VALUE result = rb_ary_new();
	rb_ary_push(result,DBL2NUM(_self->d_width.d_scale));
	rb_ary_push(result,DBL2NUM(_self->d_width.d_offset));

	rb_ary_push(result,DBL2NUM(_self->d_height.d_scale));
	rb_ary_push(result,DBL2NUM(_self->d_height.d_offset));
...
void Init_CeguiUSize(VALUE rb_mCegui)
{
	...
	rb_define_method(rb_cCeguiUSize,"to_s",RUBY_METHOD_FUNC(_to_s),0);
...
//	rb_define_method(rb_cCeguiUSize,"-",RUBY_METHOD_FUNC(_minus),1);
	...
	rb_define_method(rb_cCeguiUSize,"==",RUBY_METHOD_FUNC(_equal),1);
